...
**qwen3.5-plus Responses API 全链路修复**（2026-06-13）：
- `core/llm.py`：3 个串联 bug 一并修复
  - `_convert_tools_to_responses_format()` 把 Chat Completions 嵌套 schema `{type, function: {name,...}}` 转换为 Responses API 顶层平铺格式（修复 `tool schema invalid`）
  - `_call_responses_api` 把 `role="tool"` 转成 `function_call_output` item、assistant 带 tool_calls 转成 `function_call` items（修复 `Invalid role: tool`）
  - `_parse_responses_output` 修正 function_call 解析路径，从 `item.get("name")` 而非嵌套字段（修复 `Invalid 'name': required`）
- 端到端验证：跑"气固界面耦合新假设"任务，成功生成 7.6KB 报告（commit `d926297`）

**System Prompt 增强（为下周切 Llama-3.1-8B 准备）**（2026-06-13）：
- `core/role.py`：**Rule 14** —— 明确区分机理/数值/文献三类问题的处理逻辑
  - 机理问题：可直接答，工具可选
  - 数值问题：必须走工具
  - 文献引用：DOI 必须来自工具返回
- `core/role.py`：**Rule 15** —— Three-Step CoT 强制（THINK → ACT → SYNTHESIZE）
- `core/role.py`：**Rule 16** —— Self-Verification Before Final Output（3 问自检）
- `core/role.py`：**Few-shot examples** —— System prompt 内置 3 个示范问答（机理/数值/文献），针对 Llama 系列对 few-shot 更敏感的优化
- `core/orchestrator.py`：**Self-consistency voting** —— `--self-consistency N` CLI 参数（默认 1=禁用，3+=启用）
  - 算法：采样 N-1 个 core takeaway 变体 + 1 次投票，选出与多数一致的答案
  - 论文基础：Wang et al. 2022 "Self-Consistency Improves CoT Reasoning"
  - 推荐场景：8B 开源模型切换时开启
- System prompt 长度：~3KB → 9.3KB
- commit `0adafd0`

**API Key 安全审计 + `.gitignore` 加固**（2026-06-13）：
- 发现 `_test_fc.py` 第 5 行硬编码了真实百炼 API Key，且 README 2026-06-11 记录的"已轮换"实际未生效（同一 key 仍在 `.env` 中）
- 已在百炼控制台重置 key；`_test_fc.py` 改为从环境变量 / `.env` 自动加载
- `05_AI_Agent/.gitignore` + 根 `.gitignore` 加固兜底规则（`**/*secret*` / `**/*credential*` / `**/*password*` / `**/*token*` + 测试/调试脚本排除）
- 修复 `outputs/` 目录手抖写成 `exports/` 的旧 bug，从 git index 移除 13 个 outputs 历史跟踪文件
- 父目录 `.gitignore` 加 `99_archive/` 默认忽略 + `*.pdf` 排除（防根目录将来开 git 时推 1GB+）
- 写入 `07_项目文档/推送前自检checklist.md`
...
# Self-consistency 投票（推荐用于 8B 等开源模型；默认 1=禁用）
python run_agent.py --llm vllm_local --self-consistency 3 --critique-rounds 2 -t "评估 SiO₂ 表面催化复合系数的温度依赖性"
...
# Self-consistency voting (recommended for open-source 8B models; default 1=disabled)
python run_agent.py --llm vllm_local --self-consistency 3 --critique-rounds 2 -t "Evaluate temperature dependence of SiO2 catalytic recombination coefficient"
